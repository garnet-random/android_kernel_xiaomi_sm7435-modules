...
int mi_dsi_display_set_mipi_rw(void *display, char *buf) {
  ...
  /* check buffer's fist num to find whether set LP/HS flag to send command:
   * (00: LP)/(01:HS) */
  if (buffer[0] <= 0x01) {
    if (buffer[0] == 0x00)
...
int mi_dsi_display_read_panel_build_id(struct dsi_display *display) {
  ...
  if (!rc && !state) {
    pr_warn_ratelimited(
        "DSI Command xfer attempted while device is in suspend state\n");
    return -EPERM;
    ;
  }
  if (rc || !state) {
  ...
  if (display_id == MI_DISP_PRIMARY) {
    ...
    for (i = 0; i < gray_scale_size; i++) {
      if (i < gray_scale_size - 1)
        len =
            snprintf(&buf[index], size, "0x%02x,", mi_cfg->gray_scale_info[i]);
      else
        len =
            snprintf(&buf[index], size, "0x%02x\n", mi_cfg->gray_scale_info[i]);
...
int mi_dsi_display_get_doze_brightness(void *display, u32 *doze_brightness) {
  ...
  return mi_dsi_panel_get_doze_brightness(dsi_display->panel, doze_brightness);
}
...
int mi_dsi_display_set_brightness_clone(void *display, u32 brightness_clone) {
  ...
  ret = mi_dsi_panel_set_brightness_clone(dsi_display->panel, brightness_clone);
...
ssize_t mi_dsi_display_get_hw_vsync_info(void *display, char *buf,
                                         size_t size) {
  struct dsi_display *dsi_display = (struct dsi_display *)display;
...
mi_dsi_display_read_manufacturer_info(struct dsi_display *display,
  ...
  if (manufacturertime_offset != -1 && manufacturertime_len != -1) {
    rdlen = (rdlen > manufacturertime_offset + manufacturertime_len)
                ? rdlen
  ...
  if (rc < 0) {
    ...
    if (wp_offset != -1 && wp_len != -1) {
      info->wp_info_len = mi_dsi_display_parse_manufacturer_info(
          wp_buf, info_buf_size, rdbuf, wp_offset, wp_len);
    } else {
      info->wp_info_len = 0;
    }
    if (maxbrightness_offset != -1 && maxbrightness_len != -1) {
      info->max_brightness_len = mi_dsi_display_parse_manufacturer_info(
          maxbrightness_buf, info_buf_size, rdbuf, maxbrightness_offset,
          maxbrightness_len);
    } else {
      info->max_brightness_len = 0;
    }
    if (manufacturertime_offset != -1 && manufacturertime_len != -1) {
      info->manufacturer_time_len = mi_dsi_display_parse_manufacturer_info(
          manufacturertime_buf, info_buf_size, rdbuf, manufacturertime_offset,
          manufacturertime_len);
    } else {
...
MODULE_PARM_DESC(
    cell_id, "msm_drm.cell_id=<cell id> while <cell id> is 'cell id info' ");
