...
void mi_disp_dbg(const char *format, ...) {
  ...
  printk(KERN_DEBUG "[" DISP_NAME ":%ps] %pV", __builtin_return_address(0),
         &vaf);
...
void mi_disp_local_time_printk(const char *level, const char *format, ...) {
  ...
  printk("%s"
         ...
         level, __builtin_return_address(0), tm.tm_year + 1900, tm.tm_mon + 1,
         tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, tv.tv_nsec / 1000, &vaf);
