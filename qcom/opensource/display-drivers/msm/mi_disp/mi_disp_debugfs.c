...
u32 mi_get_backlight_log_mask(void) { return disp_debugfs.backlight_log_mask; }
...
static int mi_disp_debugfs_debug_log_init(void) {
  ...
  debugfs_create_bool(DEBUG_LOG_DEBUGFS_NAME, S_IRUGO | S_IWUSR,
                      disp_core->debugfs_dir, &disp_debugfs.debug_log_enabled);
...
static int mi_disp_debugfs_esd_sw_init(void *d_display, int disp_id) {
  ...
  if (is_support_disp_id(disp_id)) {
    disp_debugfs.esd_sw[disp_id] = debugfs_create_file(
        esd_sw_str[disp_id], S_IWUGO, disp_core->debugfs_dir, dd_ptr,
        &esd_sw_debugfs_fops);
...
static int mi_disp_debugfs_fod_test_open(struct inode *inode,
                                         struct file *file) {
  struct disp_display *dd_ptr = inode->i_private;
...
static ssize_t mi_disp_debugfs_fod_test_write(struct file *file,
  ...
  for (i = 0; i < retry_count; i++) {
    mi_disp_set_local_hbm(dd_ptr->disp_id,
                          LHBM_TARGET_BRIGHTNESS_WHITE_1000NIT);
    usleep_range((down_sleep_ms * 1000), (down_sleep_ms * 1000) + 10);
    mi_disp_set_local_hbm(dd_ptr->disp_id,
                          LHBM_TARGET_BRIGHTNESS_OFF_FINGER_UP);
    usleep_range((up_sleep_ms * 1000), (up_sleep_ms * 1000) + 10);
  }
...
static int mi_disp_debugfs_fod_test_init(void *d_display, int disp_id) {
  ...
  if (is_support_disp_id(disp_id)) {
    disp_debugfs.fod_test = debugfs_create_file(FOD_TEST_DEBUGFS_NAME, S_IWUGO,
                                                disp_core->debugfs_dir, dd_ptr,
                                                &fod_test_debugfs_fops);
    if (!disp_debugfs.fod_test) {
      DISP_ERROR("create debugfs entry failed for %s\n", FOD_TEST_DEBUGFS_NAME);
      ret = -ENODEV;
    } else {
