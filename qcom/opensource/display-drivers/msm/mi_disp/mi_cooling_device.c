...
static int mi_sde_cdev_set_cur_brightness(struct thermal_cooling_device *cdev,
  ...
  if (mi_sde_cdev->panel->mi_cfg.thermal_dimming_enabled) {
    ...
    DISP_TIME_INFO(
        "thermal dimming:set thermal_brightness_limit to %d percent\n", state);
  } else {
    brightness_lvl =
        mi_sde_cdev->panel->mi_cfg.max_brightness_clone * state / 200;
...
static int mi_sde_backlight_cooling_cb(struct notifier_block *nb,
  ...
  mi_dsi_display_set_brightness_clone(
      c_conn->display, dsi_display->panel->mi_cfg.user_brightness_clone);
  return 0;
}
...
int mi_backlight_cdev_register(struct sde_cdev *disp_cdev, struct device *dev,
  ...
  disp_cdev->cdev = thermal_of_cooling_device_register(
      dev->of_node, bl_node_name, disp_cdev, &mi_sde_cdev_ops);
...
int mi_sde_backlight_setup(struct sde_connector *c_conn, struct device *dev,
                           struct backlight_device *bd) {
  ...
  rc = mi_backlight_cdev_register(&mi_sde_cdev->sde_cdev, dev, bd,
                                  &mi_sde_cdev->n);
  if (rc) {
