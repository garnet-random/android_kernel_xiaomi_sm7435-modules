...
static void wm_adsp_buf_free(struct list_head *list) {
  while (!list_empty(list)) {
    struct wm_adsp_buf *buf = list_first_entry(list, struct wm_adsp_buf, list);
    list_del(&buf->list);
    kfree(buf->buf);
    kfree(buf);
  }
...
static const struct wm_adsp_fw_caps trace_caps[] = {
    {
        ...
        .desc =
            {
                .max_ch = 8,
                .sample_rates = {4000, 8000, 11025, 12000, 16000, 22050, 24000,
...
static ssize_t wm_adsp_debugfs_wmfw_read(struct file *file,
  ...
  if (!dsp->wmfw_file_name || !dsp->booted)
    ...
    ret = simple_read_from_buffer(user_buf, count, ppos, dsp->wmfw_file_name,
...
static ssize_t wm_adsp_debugfs_bin_read(struct file *file,
  ...
  if (!dsp->bin_file_name || !dsp->booted)
    ...
    ret = simple_read_from_buffer(user_buf, count, ppos, dsp->bin_file_name,
...
static void wm_adsp2_init_debugfs(struct wm_adsp *dsp,
  ...
  for (i = 0; i < ARRAY_SIZE(wm_adsp_debugfs_fops); ++i)
    debugfs_create_file(wm_adsp_debugfs_fops[i].name, 0444, root, dsp,
...
static void wm_adsp2v2_show_fw_status(struct wm_adsp *dsp) {
  ...
  adsp_dbg(dsp, "FW SCRATCH 0:0x%x 1:0x%x 2:0x%x 3:0x%x\n", offs[0] & 0xFFFF,
           offs[0] >> 16, offs[1] & 0xFFFF, offs[1] >> 16);
}
...
static int wm_coeff_info(struct snd_kcontrol *kctl,
  ...
  struct soc_bytes_ext *bytes_ext = (struct soc_bytes_ext *)kctl->private_value;
...
static int wm_coeff_write_acked_control(struct wm_coeff_ctl *ctl,
  ...
  adsp_dbg(dsp, "Sending 0x%x to acked control alg 0x%x %s:0x%x\n", event_id,
           ctl->alg_region.alg, wm_adsp_mem_region_name(ctl->alg_region.type),
           ctl->offset);
...
static int wm_coeff_write_ctrl_raw(struct wm_coeff_ctl *ctl, const void *buf,
                                   size_t len) {
  struct wm_adsp *dsp = ctl->dsp;
  void *scratch, *temp;
...
static int wm_coeff_write_ctrl(struct wm_coeff_ctl *ctl, const void *buf,
                               size_t len) {
  int ret = 0;
...
static int wm_coeff_read_ctrl_raw(struct wm_coeff_ctl *ctl, void *buf,
                                  size_t len) {
  struct wm_adsp *dsp = ctl->dsp;
  void *scratch, *temp;
...
static int wm_coeff_sync_controls(struct wm_adsp *dsp) {
  ...
  list_for_each_entry(ctl, &dsp->ctl_list, list) {
    ...
    if (ctl->set && !(ctl->flags & WMFW_CTL_FLAG_VOLATILE)) {
      ret = wm_coeff_write_ctrl_raw(ctl, ctl->cache, ctl->len);
      if (ret < 0)
        return ret;
    }
  }
...
static void wm_adsp_signal_event_controls(struct wm_adsp *dsp,
  ...
  list_for_each_entry(ctl, &dsp->ctl_list, list) {
    ...
    if (ret)
      adsp_warn(dsp, "Failed to send 0x%x event to alg 0x%x (%d)\n", event,
                ctl->alg_region.alg, ret);
  }
}
...
static int wm_adsp_create_control(struct wm_adsp *dsp,
  ...
  case 0:
  case 1:
    snprintf(name, SNDRV_CTL_ELEM_ID_NAME_MAXLEN, "%s %s %x", dsp->name,
             region_name, alg_region->alg);
    subname = NULL; /* don't append subname */
    break;
  case 2:
    ret = scnprintf(name, SNDRV_CTL_ELEM_ID_NAME_MAXLEN, "%s%c %.12s %x",
                    dsp->name, *region_name, fw_txt, alg_region->alg);
    break;
  default:
    ret = scnprintf(name, SNDRV_CTL_ELEM_ID_NAME_MAXLEN, "%s %.12s %x",
                    dsp->name, fw_txt, alg_region->alg);
    break;
  }
  ...
  if (subname) {
    ...
    snprintf(name + ret, SNDRV_CTL_ELEM_ID_NAME_MAXLEN - ret, " %.*s",
...
int wm_adsp_handle_fw_event(struct wm_adsp *dsp) {
  ...
  list_for_each_entry(ctl, &dsp->ctl_list, list) {
    if (ctl->type != WMFW_CTL_TYPE_FWEVENT)
      continue;
    if (!dsp->fwevent_cb) {
      adsp_err(dsp, "FW event callback not registered: %s\n", ctl->name);
      return -EINVAL;
    }
...
static inline void wm_coeff_parse_alg(struct wm_adsp *dsp, const u8 **data,
  ...
  default:
    ...
    blk->name_len = wm_coeff_parse_string(sizeof(u8), data, &blk->name);
...
static inline void wm_coeff_parse_coeff(struct wm_adsp *dsp, const u8 **data,
  ...
  default:
    ...
    blk->name_len = wm_coeff_parse_string(sizeof(u8), &tmp, &blk->name);
  ...
  for (i = 0; i < alg_blk.ncoeff; i++) {
    ...
    case WMFW_CTL_TYPE_FWEVENT:
      ret = wm_adsp_check_coeff_flags(
          dsp, &coeff_blk,
          WMFW_CTL_FLAG_SYS | WMFW_CTL_FLAG_VOLATILE | WMFW_CTL_FLAG_WRITEABLE |
              WMFW_CTL_FLAG_READABLE,
          0);
      if (ret)
        return -EINVAL;
    ...
    case WMFW_CTL_TYPE_HOST_BUFFER:
      ret = wm_adsp_check_coeff_flags(
          dsp, &coeff_blk,
          WMFW_CTL_FLAG_SYS | WMFW_CTL_FLAG_VOLATILE | WMFW_CTL_FLAG_READABLE,
          0);
      if (ret)
        return -EINVAL;
      break;
    ...
    ret = wm_adsp_create_control(
        dsp, &alg_region, coeff_blk.offset, coeff_blk.len, coeff_blk.name,
        coeff_blk.name_len, coeff_blk.flags, coeff_blk.ctl_type);
    if (ret < 0)
...

{
  ...
  while (remain > 0) {
    ...
    ret = regmap_raw_write_async(dsp->regmap, reg, buf->buf, to_write);
    if (ret != 0) {
...
static bool wm_adsp_validate_version(struct wm_adsp *dsp,
...
static bool wm_halo_validate_version(struct wm_adsp *dsp,
                                     unsigned int version) {
  switch (version) {
  case 3:
    return true;
...
static int wm_adsp_load(struct wm_adsp *dsp) {
  ...
  while (pos < firmware->size && sizeof(*region) < firmware->size - pos) {
    ...
    case WMFW_NAME_TEXT:
      ...
      text = kzalloc(le32_to_cpu(region->len) + 1, GFP_KERNEL);
      break;
    ...
    case WMFW_HALO_YM_PACKED:
      ...
      break;
    default:
      adsp_warn(dsp, "%s.%d: Unknown region type %x at %d(%x)\n", file, regions,
                type, pos, pos);
      break;
    }
    ...
    if (le32_to_cpu(region->len) > firmware->size - pos - sizeof(*region)) {
      adsp_err(dsp, "%s.%d: %s region len %d bytes exceeds file length %zu\n",
               file, regions, region_name, le32_to_cpu(region->len),
               firmware->size);
      ret = -EINVAL;
      goto out_fw;
    }
    ...
    if (reg) {
      ret = wm_adsp_write_blocks(dsp, region->data, le32_to_cpu(region->len),
                                 reg, &buf_list, burst_multiple);
      if (ret != 0) {
        adsp_err(dsp, "%s.%d: Failed writing data at %d in %s: %d\n", file,
                 regions, offset, region_name, ret);
        goto out_fw;
      }
    }
...
static struct wm_coeff_ctl *wm_adsp_get_ctl(struct wm_adsp *dsp,
  ...
  list_for_each_entry(pos, &dsp->ctl_list, list) {
    ...
    {
            adsp_err(dsp, "pos->subname is %s, pos->subname_len is %d,
    pos->enabled is %d\n", pos->subname, pos->subname_len, pos->enabled);
            adsp_err(dsp, "pos->alg_region.alg is %d, alg is %d,
    pos->alg_region.type is %d, type is %d\n", pos->alg_region.alg, alg,
...
int wm_adsp_write_ctl(struct wm_adsp *dsp, const char *name, int type,
  ...
  if (dsp->component->name_prefix)
    ...
    snprintf(ctl_name, SNDRV_CTL_ELEM_ID_NAME_MAXLEN, "%s", ctl->name);
  ...
  snd_ctl_notify(dsp->component->card->snd_card, SNDRV_CTL_EVENT_MASK_VALUE,
                 &kcontrol->id);
...
static void
wm_adsp_ctl_fixup_base(struct wm_adsp *dsp,
                       const struct wm_adsp_alg_region *alg_region) {
  struct wm_coeff_ctl *ctl;
...
static void wm_adsp_free_alg_regions(struct wm_adsp *dsp) {
  ...
  while (!list_empty(&dsp->alg_regions)) {
    alg_region =
        list_first_entry(&dsp->alg_regions, struct wm_adsp_alg_region, list);
...
static void wmfw_v3_parse_id_header(struct wm_adsp *dsp,
  ...
  adsp_info(
      ...
      dsp->fw_vendor_id, (dsp->fw_id_version & 0xff0000) >> 16,
      (dsp->fw_id_version & 0xff00) >> 8, dsp->fw_id_version & 0xff, nalgs);
}
...
static int wm_adsp1_setup_algs(struct wm_adsp *dsp) {
  ...
  ret = regmap_raw_read(dsp->regmap, mem->base, &adsp1_id, sizeof(adsp1_id));
  if (ret != 0) {
  ...
  alg_region =
      wm_adsp_create_region(dsp, WMFW_ADSP1_ZM, adsp1_id.fw.id, adsp1_id.zm);
  if (IS_ERR(alg_region))
    return PTR_ERR(alg_region);

  alg_region =
      wm_adsp_create_region(dsp, WMFW_ADSP1_DM, adsp1_id.fw.id, adsp1_id.dm);
  if (IS_ERR(alg_region))
    return PTR_ERR(alg_region);
  ...
  for (i = 0; i < n_algs; i++) {
    adsp_info(dsp, "%d: ID %x v%d.%d.%d DM@%x ZM@%x\n", i,
              ...
              be32_to_cpu(adsp1_alg[i].dm), be32_to_cpu(adsp1_alg[i].zm));

    alg_region = wm_adsp_create_region(dsp, WMFW_ADSP1_DM, adsp1_alg[i].alg.id,
                                       adsp1_alg[i].dm);
    if (IS_ERR(alg_region)) {
    ...
    if (dsp->fw_ver == 0) {
      if (i + 1 < n_algs) {
        ...
        len *= 4;
        wm_adsp_create_control(dsp, alg_region, 0, len, NULL, 0, 0,
    ...
    alg_region = wm_adsp_create_region(dsp, WMFW_ADSP1_ZM, adsp1_alg[i].alg.id,
                                       adsp1_alg[i].zm);
    if (IS_ERR(alg_region)) {
      ret = PTR_ERR(alg_region);
      goto out;
...
static int wm_adsp2_setup_algs(struct wm_adsp *dsp) {
  ...
  ret = regmap_raw_read(dsp->regmap, mem->base, &adsp2_id, sizeof(adsp2_id));
  if (ret != 0) {
    adsp_err(dsp, "Failed to read algorithm info: %d\n", ret);
    return ret;
  }
  ...
  alg_region =
      wm_adsp_create_region(dsp, WMFW_ADSP2_XM, adsp2_id.fw.id, adsp2_id.xm);
  if (IS_ERR(alg_region))
    return PTR_ERR(alg_region);

  alg_region =
      wm_adsp_create_region(dsp, WMFW_ADSP2_YM, adsp2_id.fw.id, adsp2_id.ym);
  if (IS_ERR(alg_region))
    return PTR_ERR(alg_region);

  alg_region =
      wm_adsp_create_region(dsp, WMFW_ADSP2_ZM, adsp2_id.fw.id, adsp2_id.zm);
  if (IS_ERR(alg_region))
    return PTR_ERR(alg_region);
  ...
  for (i = 0; i < n_algs; i++) {
    adsp_info(dsp, "%d: ID %x v%d.%d.%d XM@%x YM@%x ZM@%x\n", i,
              ...
              be32_to_cpu(adsp2_alg[i].xm), be32_to_cpu(adsp2_alg[i].ym),
              be32_to_cpu(adsp2_alg[i].zm));

    alg_region = wm_adsp_create_region(dsp, WMFW_ADSP2_XM, adsp2_alg[i].alg.id,
                                       adsp2_alg[i].xm);
    if (IS_ERR(alg_region)) {
      ret = PTR_ERR(alg_region);
      goto out;
    ...
    alg_region = wm_adsp_create_region(dsp, WMFW_ADSP2_YM, adsp2_alg[i].alg.id,
                                       adsp2_alg[i].ym);
    if (IS_ERR(alg_region)) {
      ret = PTR_ERR(alg_region);
      goto out;
...
static int wm_halo_setup_algs(struct wm_adsp *dsp) {
  ...
  ret = regmap_raw_read(dsp->regmap, mem->base, &halo_id, sizeof(halo_id));
  if (ret != 0) {
    adsp_err(dsp, "Failed to read algorithm info: %d\n", ret);
    return ret;
  }
  ...
  for (i = 0; i < n_algs; i++) {
    ...
    ret = wm_halo_create_regions(dsp, halo_alg[i].alg.id, halo_alg[i].xm_base,
                                 halo_alg[i].ym_base);
    if (ret)
      goto out;
  }
...
static int wm_vpu_setup_algs(struct wm_adsp *vpu) {
  ...
  alg_region =
      wm_adsp_create_region(vpu, WMFW_VPU_DM, vpu_id.fw.id, vpu_id.dm_base);
  if (IS_ERR(alg_region))
    return PTR_ERR(alg_region);
  ...
  for (i = 0; i < n_algs; i++) {
    ...
    alg_region = wm_adsp_create_region(vpu, WMFW_VPU_DM, vpu_alg[i].alg.id,
                                       vpu_alg[i].dm_base);
    if (IS_ERR(alg_region)) {
      ret = PTR_ERR(alg_region);
      goto out;
...
static int wm_adsp_load_coeff(struct wm_adsp *dsp) {
  ...
  if (dsp->tuning_has_prefix || dsp->component->name_prefix)
    snprintf(file, PAGE_SIZE, "%s-%s-%s-%s.bin", dsp->component->name_prefix,
  ...
  adsp_dbg(dsp, "%s: v%d.%d.%d\n", file, (le32_to_cpu(hdr->ver) >> 16) & 0xff,
           (le32_to_cpu(hdr->ver) >> 8) & 0xff, le32_to_cpu(hdr->ver) & 0xff);
  ...
  while (pos < firmware->size && sizeof(*blk) < firmware->size - pos) {
    ...
    adsp_dbg(dsp, "%s.%d: %x v%d.%d.%d\n", file, blocks, le32_to_cpu(blk->id),
             ...
             (le32_to_cpu(blk->ver) >> 8) & 0xff, le32_to_cpu(blk->ver) & 0xff);
    ...
    case WMFW_VPU_DM:
      adsp_dbg(dsp, "%s.%d: %d bytes in %x for %x\n", file, blocks,
               le32_to_cpu(blk->len), type, le32_to_cpu(blk->id));
      ...
      alg_region = wm_adsp_find_alg_region(dsp, type, le32_to_cpu(blk->id));
      if (alg_region) {
        reg = alg_region->base;
    ...
    if (reg) {
      if (le32_to_cpu(blk->len) > firmware->size - pos - sizeof(*blk)) {
        adsp_err(dsp, "%s.%d: %s region len %d bytes exceeds file length %zu\n",
                 file, blocks, region_name, le32_to_cpu(blk->len),
                 firmware->size);
        ret = -EINVAL;
        goto out_fw;
      }
      ...
      ret = wm_adsp_write_blocks(dsp, blk->data, le32_to_cpu(blk->len), reg,
                                 &buf_list, burst_multiple);
      if (ret != 0) {
        adsp_err(dsp, "%s.%d: Failed to write to %x in %s: %d\n", file, blocks,
                 reg, region_name, ret);
      }
    }
...
static int wm_adsp_create_name(struct wm_adsp *dsp) {
  ...
  if (!dsp->name) {
    dsp->name = devm_kasprintf(dsp->dev, GFP_KERNEL, "DSP%d", dsp->num);
    if (!dsp->name)
      return -ENOMEM;
  }
...
int wm_adsp1_event(struct snd_soc_dapm_widget *w, struct snd_kcontrol *kcontrol,
  ...
  case SND_SOC_DAPM_POST_PMU:
    regmap_update_bits(dsp->regmap, dsp->base + ADSP1_CONTROL_30, ADSP1_SYS_ENA,
                       ADSP1_SYS_ENA);
    ...
    if (dsp->sysclk_reg) {
      ...
      ret = regmap_update_bits(dsp->regmap, dsp->base + ADSP1_CONTROL_31,
  ...
  case SND_SOC_DAPM_PRE_PMD:
    ...
    list_for_each_entry(ctl, &dsp->ctl_list, list) ctl->enabled = 0;
...
static void wm_adsp2_disable_memory(struct wm_adsp *dsp) {
  regmap_update_bits(dsp->regmap, dsp->base + ADSP2_CONTROL, ADSP2_MEM_ENA, 0);
}
...
static void wm_adsp2_disable_core(struct wm_adsp *dsp) {
  ...
  regmap_update_bits(dsp->regmap, dsp->base + ADSP2_CONTROL, ADSP2_SYS_ENA, 0);
}
...
static int wm_halo_set_rate_block(struct wm_adsp *dsp, unsigned int rate_base,
  ...
  for (i = 0; i < n_rates; ++i) {
    ...
    ret = regmap_update_bits(dsp->regmap, addr + (i * 8), HALO_DSP_RATE_MASK,
                             val);
    if (ret) {
...
int wm_adsp2_set_dspclk(struct snd_soc_dapm_widget *w, unsigned int freq) {
  ...
  ret = regmap_update_bits(dsp->regmap, dsp->base + ADSP2_CLOCKING,
                           ADSP2_CLK_SEL_MASK, freq << ADSP2_CLK_SEL_SHIFT);
  if (ret)
    adsp_err(dsp, "Failed to set clock rate: %d\n", ret);
...
static int wm_halo_start_core(struct wm_adsp *dsp) {
  ...
  ret = wm_halo_set_rate_block(dsp, HALO_SAMPLE_RATE_RX1, dsp->n_rx_channels,
                               dsp->rx_rate_cache);
  if (ret) {
  ...
  ret = wm_halo_set_rate_block(dsp, HALO_SAMPLE_RATE_TX1, dsp->n_tx_channels,
                               dsp->tx_rate_cache);
  if (ret) {
...
void wm_adsp2_remove(struct wm_adsp *dsp) {
  ...
  while (!list_empty(&dsp->ctl_list)) {
    ctl = list_first_entry(&dsp->ctl_list, struct wm_coeff_ctl, list);
    list_del(&ctl->list);
...
wm_adsp_compr_frag_words(struct wm_adsp_compr *compr) {
...
static int wm_adsp_read_data_block(struct wm_adsp *dsp, int mem_type,
  ...
  ret = regmap_raw_read(dsp->regmap, reg, data, sizeof(*data) * num_words);
  if (ret < 0)
    return ret;
...
static int wm_adsp_buffer_populate(struct wm_adsp_compr_buf *buf) {
  ...
  buf->regions = kcalloc(buf->num_regions, sizeof(*buf->regions), GFP_KERNEL);
  if (!buf->regions)
    return -ENOMEM;
  ...
  for (i = 0; i < buf->num_regions; ++i) {
    ...
    ret = wm_adsp_buffer_read(buf, buf->region_defs[i].size_offset, &offset);
    if (ret < 0)
      return ret;

    region->cumulative_size = offset;

    compr_dbg(buf, "region=%d type=%d base=%08x off=%08x size=%08x\n", i,
...
static int wm_adsp_buffer_parse_legacy(struct wm_adsp *dsp) {
  ...
  for (i = 0; i < 5; ++i) {
    ret = wm_adsp_read_data_word(dsp, WMFW_ADSP2_XM, addr, &buf->host_buf_ptr);
    if (ret < 0)
      return ret;
...
static int wm_adsp_buffer_parse_coeff(struct wm_coeff_ctl *ctl) {
  ...
  ret = regmap_raw_read(ctl->dsp->regmap, reg, &coeff_v1, sizeof(coeff_v1));
  if (ret < 0)
    return ret;
  ...
  if (val > HOST_BUF_COEFF_SUPPORTED_COMPAT_VER) {
    adsp_err(ctl->dsp, "Host buffer coeff ver %u > supported version %u\n", val,
...
int wm_adsp_compr_trigger(struct snd_compr_stream *stream, int cmd) {
  ...
  case SNDRV_PCM_TRIGGER_START:
    ...
    ret = wm_adsp_buffer_write(compr->buf, HOST_BUFFER_FIELD(high_water_mark),
...
static int wm_adsp_buffer_update_avail(struct wm_adsp_compr_buf *buf) {
  ...
  if (buf->read_index < 0) {
    ret = wm_adsp_buffer_read(buf, HOST_BUFFER_FIELD(next_read_index),
                              &next_read_index);
    if (ret < 0)
      return ret;
  ...
  compr_dbg(buf, "readindex=0x%x, writeindex=0x%x, avail=%d\n", buf->read_index,
            write_index, avail * buf->dsp->data_word_size);
...
int wm_adsp_compr_handle_irq(struct wm_adsp *dsp) {
  ...
  list_for_each_entry(buf, &dsp->buffer_list, list) {
    ...
    ret =
        wm_adsp_buffer_read(buf, HOST_BUFFER_FIELD(irq_count), &buf->irq_count);
    if (ret < 0) {
...
static int wm_adsp_buffer_reenable_irq(struct wm_adsp_compr_buf *buf) {
  ...
  return wm_adsp_buffer_write(buf, HOST_BUFFER_FIELD(irq_ack), buf->irq_count);
}
...
static int wm_adsp_buffer_capture_block(struct wm_adsp_compr *compr,
  ...
  adsp_addr =
      buf->regions[i].base_addr + (buf->read_index - buf->regions[i].offset);
...
irqreturn_t wm_adsp2_bus_error(int irq, void *data) {
  ...
  if (val & (ADSP2_ADDR_ERR_MASK | ADSP2_REGION_LOCK_ERR_MASK)) {
    ...
    ret = regmap_read(regmap, dsp->base + ADSP2_PMEM_ERR_ADDR_XMEM_ERR_ADDR,
                      &val);
    if (ret) {
    ...
    adsp_err(dsp, "pmem error address = 0x%x\n",
             (val & ADSP2_PMEM_ERR_ADDR_MASK) >> ADSP2_PMEM_ERR_ADDR_SHIFT);
  }
...
irqreturn_t wm_halo_bus_error(int irq, void *data) {
  ...
  ret =
      regmap_read(regmap, dsp->base_sysinfo + HALO_AHBM_WINDOW_DEBUG_1, fault);
  if (ret) {
  ...
  adsp_warn(
      dsp, "AHB: STATUS: 0x%x ADDR: 0x%x\n", *fault & HALO_AHBM_FLAGS_ERR_MASK,
      (*fault & HALO_AHBM_CORE_ERR_ADDR_MASK) >> HALO_AHBM_CORE_ERR_ADDR_SHIFT);

  ret =
      regmap_read(regmap, dsp->base_sysinfo + HALO_AHBM_WINDOW_DEBUG_0, fault);
  if (ret) {
