...
int mi_dsi_panel_parse_esd_gpio_config(struct dsi_panel *panel) {
  ...
  mi_cfg->esd_err_irq_gpio = of_get_named_gpio_flags(
      ...
      (enum of_gpio_flags *)&(mi_cfg->esd_err_irq_flags));
  ...
  if (!strcmp(panel->name,
    ...
    mi_cfg->esd_err_irq_gpio_second = of_get_named_gpio_flags(
        ...
        (enum of_gpio_flags *)&(mi_cfg->esd_err_irq_flags_second));
    if (gpio_is_valid(mi_cfg->esd_err_irq_gpio_second)) {
      mi_cfg->esd_err_irq_second = gpio_to_irq(mi_cfg->esd_err_irq_gpio_second);
      rc = gpio_request(mi_cfg->esd_err_irq_gpio_second,
                        "esd_err_irq_gpio_second");
      if (rc)
  ...
  if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == L3_PANEL_PA ||
    ...
    if (!IS_ERR(lhbm_ptr) && item_size > 0) {
      ...
      for (i = 1; i < item_size; i += 2) {
        tmp =
            ((mi_cfg->lhbm_rgb_param[i - 1]) << 8) | mi_cfg->lhbm_rgb_param[i];
...
static void mi_dsi_panel_parse_lockdown_config(struct dsi_panel *panel) {
  ...
  if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == M80_PANEL_PA) {
    ...
    lockdown_ptr = qcom_smem_get(QCOM_SMEM_HOST_ANY,
                                 SMEM_SW_DISPLAY_LOCKDOWN_TABLE, &item_size);
    ...
    for (i = 0; i < 8; i++) {
      DISP_ERROR(
          "M80 lockdown data mi_cfg->lockdown_cfg.lockdown_param[%d] = 0x%0x\n",
...
static void mi_dsi_panel_parse_gray_scale_config(struct dsi_panel *panel) {
  ...
  if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == L12_PANEL_PA ||
      mi_get_panel_id(panel->mi_cfg.mi_panel_id) == L12_PANEL_PB) {
    gray_scale_ptr = qcom_smem_get(
        QCOM_SMEM_HOST_ANY, SMEM_SW_DISPLAY_GRAY_SCALE_TABLE, &item_size);
...
static void mi_dsi_panel_parse_flat_config(struct dsi_panel *panel) {
  ...
  mi_cfg->flat_update_gamma_zero =
      utils->read_bool(utils->data, "mi,flat-need-update-gamma-zero");
  ...
  mi_cfg->flat_update_several_gamma =
      utils->read_bool(utils->data, "mi,flat-need-update-several-gamma");
...
static int mi_dsi_panel_parse_backlight_config(struct dsi_panel *panel) {
  ...
  mi_cfg->dimming_need_update_speed =
      utils->read_bool(utils->data, "mi,dimming-need-update-speed");
  ...
  rc = utils->read_u32(utils->data, "mi,normal-max-brightness-clone",
  ...
  mi_cfg->thermal_dimming_enabled =
      utils->read_bool(utils->data, "mi,thermal-dimming-flag");
  if (mi_cfg->thermal_dimming_enabled) {
    DISP_INFO("thermal_dimming enabled\n");
  }

  mi_cfg->video_fps_cmdsets_enanle =
      utils->read_bool(utils->data, "mi,video-fps-cmdsets-flag");
...
int mi_dsi_panel_parse_config(struct dsi_panel *panel) {
  ...
  mi_cfg->count_hbm_by_backlight =
      utils->read_bool(utils->data, "mi,panel-count-hbm-by-backlight-flag");
  if (mi_cfg->count_hbm_by_backlight)
    DISP_INFO("panel count hbm by backlight\n");

  mi_cfg->ignore_esd_in_aod =
      utils->read_bool(utils->data, "mi,panel-ignore-esd-in-aod");
