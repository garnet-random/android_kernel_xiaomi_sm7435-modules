...
static bool
mi_flatmode_validate_status(struct dsi_display *display,
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (mi_dsi_display_cmd_read(display, cmds[i], config->status_buf,
                                lenp[i]) <= 0) {
  ...
  for (i = 0; i < len; ++i) {
    DISP_INFO(" flat mode return_buff[%d] = 0x%x, status_value[%d] = 0x%x", i,
              config->return_buf[i], i, config->status_value[i]);
