...
static int mi_disp_log_printk_func(struct log_buf *log_buf, const char *fmt,
  ...
  ts_usec %= 1000000;
  text_len =
      snprintf(text, sizeof(textbuf), "[%5lld.%06lld] ", ts_sec, ts_usec);
  text_len += vsnprintf(text + text_len, sizeof(textbuf) - text_len, fmt, args);
...
void disp_log_printk(const char *format, ...) {
  ...
  mi_disp_log_kenrel_printk("[%ps] %pV", __builtin_return_address(0), &vaf);
...
static ssize_t mi_disp_log_read(struct file *file, char __user *buf,
  ...
  if (log_buf->pos.wrap) {
    if (log_buf->pos.offset + read_len < log_buf->size) {
      if (copy_to_user(buf, &log_buf->buf[log_buf->pos.offset], read_len)) {
        ret = -EFAULT;
        goto out;
      }
    } else {
...
static ssize_t info_show(struct device *device, struct device_attribute *attr,
  ...
  log_buf = &disp_log->log[LOG_TYPE_KERNEL];
  count =
      snprintf(buf, PAGE_SIZE, "Kernel Display log parameter information:\n");
...
static struct attribute *disp_log_dev_attrs[] = {
    &dev_attr_info.attr, &dev_attr_user_buf.attr, &dev_attr_kernel_buf.attr,
    NULL};
...
int mi_disp_log_init(void) {
  ...
  disp_log->dev = device_create_with_groups(
      disp_log->class, NULL, disp_log->dev_id, disp_log, disp_log_dev_groups,
      DISP_LOG_DEVICE_NAME);
  ...
  disp_log->debug = debugfs_create_file(
      DISP_LOG_DEVICE_NAME, 0444, disp_core ? disp_core->debugfs_dir : NULL,
