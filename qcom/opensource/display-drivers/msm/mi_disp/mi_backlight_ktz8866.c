...
int ktz8866_backlight_update_status(unsigned int level) {
  ...
  if (g_ktz8866_led.HBM_enable) {
    ...
    if (exponential_bl <= BL_LEVEL_MAX) {
      ...
      exponential_bl =
          ((exponential_bl - 2048) * (2047 - normal_brightness)) / 2047 +
          normal_brightness;
    } else {
...
static int ktz8866_probe(struct i2c_client *client,
  ...
  if (id != NULL) {
    if (id->driver_data == KTZ8866_A) {
      ...
      g_ktz8866_led.HBM_enable = of_property_read_bool(
          (&g_client->dev)->of_node, "ktz8866,backlight-HBM-enable");
      if (g_ktz8866_led.HBM_enable)
...
int mi_backlight_ktz8866_init(void) { return i2c_add_driver(&ktz8866_driver); }
