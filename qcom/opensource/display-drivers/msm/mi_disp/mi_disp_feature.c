...
static int mi_disp_feature_thread_destroy(struct disp_feature *df,
                                          int disp_id) {
  int ret = 0;
  struct disp_display *dd_ptr = NULL;
...
int mi_disp_feature_attach_display(void *display, int disp_id, int intf_type) {
  ...
  dd_ptr->dev = device_create(df->class, df->pdev, 0, dd_ptr, "disp-%s-%d",
...
void mi_disp_feature_event_notify(struct disp_event *event, u8 *payload) {
  ...
  list_for_each_entry(client, &df->client_list, link) {
    ...
    DISP_DEBUG("%s display event length: %d\n",
               get_disp_id_name(event->disp_id), notify->event.base.length);
...
int mi_disp_feature_init(void) {
  ...
  ret = mi_disp_cdev_register(DISP_FEATURE_DEVICE_NAME, &disp_feature_fops,
                              &df->cdev);
  if (ret < 0) {
  ...
  df->pdev =
      device_create(df->class, NULL, df->dev_id, df, DISP_FEATURE_DEVICE_NAME);
  if (IS_ERR(df->pdev)) {
