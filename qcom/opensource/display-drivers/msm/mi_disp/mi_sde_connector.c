  ...
  if (c_conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
    ...
    if (display->panel->mi_cfg.esd_err_irq_gpio > 0) {
      rc = request_threaded_irq(
          display->panel->mi_cfg.esd_err_irq, NULL, mi_esd_err_irq_handle,
          display->panel->mi_cfg.esd_err_irq_flags, "esd_err_irq", c_conn);
      if (rc) {
    ...
    if (mi_get_panel_id(display->panel->mi_cfg.mi_panel_id) == M80_PANEL_PA) {
      if (display->panel->mi_cfg.esd_err_irq_gpio_second > 0) {
        rc = request_threaded_irq(
            display->panel->mi_cfg.esd_err_irq_second, NULL,
...
int mi_sde_connector_debugfs_esd_sw_trigger(void *display) {
  ...
  c_conn->panel_dead = true;
  DISP_ERROR("[esd-test]esd irq check failed report PANEL_DEAD conn_id: %d "
             "enc_id: %d\n",
    ...
    if (display) {
      if (mi_get_panel_id(display->panel->mi_cfg.mi_panel_id) == N16_PANEL_PC ||
        ...
        if (aod_status_restore != is_aod_exit && is_aod_exit &&
          ...
          DISP_INFO("AOD layer disappears prematurely and is inserted black in "
                    "advance\n");
          dsi_panel_acquire_panel_lock(display->panel);
...
int mi_sde_connector_update_layer_state(struct drm_connector *connector,
  ...
  cur_flags.fod_overlay_flag =
      (mi_layer_flags & MI_LAYER_FOD_HBM_OVERLAY) ? true : false;
    ...
    if (connector->connector_type == DRM_MODE_CONNECTOR_DSI) {
      ...
      if (display && mi_disp_lhbm_fod_enabled(display->panel)) {
        if ((mi_get_panel_id_by_dsi_panel(display->panel) == N16_PANEL_PB ||
          ...
          ret = mi_disp_lhbm_aod_to_normal_optimize(display,
                                                    cur_flags.fod_anim_flag);
          if (ret == -EAGAIN) {
  ...
  if (mi_cfg->flat_sync_te) {
    ...
    if (flat_mode_val != mi_cfg->flat_cfg.cur_flat_state) {
      ...
      mi_disp_feature_event_notify_by_type(
          mi_get_disp_id(dsi_display->display_type), MI_DISP_EVENT_FLAT_MODE,
          sizeof(int), flat_mode_val);
