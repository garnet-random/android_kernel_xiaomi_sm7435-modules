...
ssize_t mi_disp_read(struct file *filp, char __user *buffer, size_t count,
  ...
  for (;;) {
    ...
    if (!list_empty(&client->event_list)) {
      e = list_first_entry(&client->event_list, struct disp_pending_event,
                           link);
  ...
  if (is_support_disp_id(disp_id)) {
    ...
    if (dd_ptr->intf_type == MI_INTF_DSI) {
      DISP_INFO("%s display doze_brightness = %d\n", get_disp_id_name(disp_id),
                doze_brightness);
      if (req->base.flag == MI_DISP_FLAG_NONBLOCK) {
        ret = mi_disp_set_doze_brightness_queue_work(dd_ptr, &doze_brightness,
...
static int mi_disp_ioctl_get_panel_info(struct disp_feature_client *client,
  ...
  if (is_support_disp_id(disp_id)) {
    dd_ptr = &df->d_display[disp_id];
    if (dd_ptr->intf_type == MI_INTF_DSI) {
      len = mi_dsi_display_read_panel_info(dd_ptr->display, panel_name,
                                           sizeof(panel_name));
      if (len > req->info_len)
        len = req->info_len;
...
static int mi_disp_ioctl_get_wp_info(struct disp_feature_client *client,
  ...
  if (is_support_disp_id(disp_id)) {
    dd_ptr = &df->d_display[disp_id];
    if (dd_ptr->intf_type == MI_INTF_DSI) {
      len = mi_dsi_display_read_wp_info(dd_ptr->display, wp_info,
                                        sizeof(wp_info));
      if (len > req->info_len)
        len = req->info_len;
...
mi_disp_ioctl_get_manufacturer_info(struct disp_feature_client *client,
  ...
  if (is_support_disp_id(disp_id)) {
    ...
    if (dd_ptr->intf_type == MI_INTF_DSI) {
      ...
      len = (info.wp_info_len > req->wp_info_len) ? req->wp_info_len
                                                  : info.wp_info_len;
      ...
      len = (info.max_brightness_len > req->max_brightness_len)
      ...
      len = (info.manufacturer_time_len > req->manufacturer_time_len)
...
static int mi_disp_ioctl_get_fps(struct disp_feature_client *client,
                                 void *data) {
  struct disp_feature *df = client->df;
...
static int mi_disp_ioctl_write_dsi_cmd(struct disp_feature_client *client,
  ...
  if (is_support_disp_id(disp_id)) {
    ...
    if (dd_ptr->intf_type == MI_INTF_DSI) {
      ...
      if (copy_from_user(ctl.tx_ptr, (void __user *)req->tx_ptr, ctl.tx_len) !=
          0) {
        ret = -EFAULT;
        goto err_free_tx;
      }
  ...
  if (is_support_disp_id(disp_id)) {
    ...
    if (dd_ptr->intf_type == MI_INTF_DSI) {
      ...
      if (recv_len <= 0 || recv_len != ctl.rx_len) {
        ...
        if (copy_to_user((void __user *)req->rx_ptr, ctl.rx_ptr, ctl.rx_len) !=
            0) {
          ret = -EFAULT;
          goto err_free_rx;
        }
...
static int mi_disp_ioctl_get_brightness(struct disp_feature_client *client,
  ...
  if (is_support_disp_id(disp_id)) {
    dd_ptr = &df->d_display[disp_id];
    if (dd_ptr->intf_type == MI_INTF_DSI) {
      ret = mi_dsi_display_get_brightness(dd_ptr->display, &req->brightness);
