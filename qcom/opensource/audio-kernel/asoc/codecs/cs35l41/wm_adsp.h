...
#define WM_ADSP1(wname, num)                                                   \
  SND_SOC_DAPM_PGA_E(wname, SND_SOC_NOPM, num, 0, NULL, 0, wm_adsp1_event,     \
...
#define WM_ADSP_FW_CONTROL(dspname, num)                                       \
  SOC_ENUM_EXT(dspname " Firmware", wm_adsp_fw_enum[num], wm_adsp_fw_get,      \
               wm_adsp_fw_put)
...
#define regmap_update_bits(map, reg, mask, val)                                \
  ({                                                                           \
    ...
    for (__i = 0; __i < 5; __i++) {                                            \
      ...
      if (__ret) {                                                             \
        pr_info(                                                               \
            "I2C retry : regmap_update_bits: %d, (0x%x, %x, %x), ret = %d\n",  \
...
#define regmap_raw_write(map, reg, val, len)                                   \
  ({                                                                           \
    ...
    for (__i = 0; __i < 5; __i++) {                                            \
      ...
      if (__ret) {                                                             \
        pr_info(                                                               \
            "I2C retry : regmap_raw_write: %d, (0x%x, %p, %d), ret = %d\n",    \
...
#define regmap_raw_write_async(map, reg, val, len)                             \
  ({                                                                           \
    ...
    for (__i = 0; __i < 5; __i++) {                                            \
      ...
      if (__ret) {                                                             \
        pr_info("I2C retry : regmap_raw_write_async: %d, (0x%x, %p, %d), ret " \
                "= %d\n",                                                      \
                __i, reg, val, len, __ret);                                    \
        usleep_range(1000, 2000);                                              \
        continue;                                                              \
